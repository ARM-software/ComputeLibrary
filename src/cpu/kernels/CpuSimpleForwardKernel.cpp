...
void CpuSimpleForwardKernel::run_op(ITensorPack &tensors, const Window &window, const ThreadInfo &info)
{
    ...
    std::cout << "src/cpu/kernels/CpuSimpleForwardKernel.cpp Runnnn" << std::endl;
    std::cout << *reinterpret_cast<float*>(src1->ptr_to_element(Coordinates(0,1))) <<std::endl;
    std::cout << *reinterpret_cast<float*>(src2->ptr_to_element(Coordinates(0,1))) <<std::endl;
    std::cout << *reinterpret_cast<float*>(src3->ptr_to_element(Coordinates(0,1))) <<std::endl;

    std::cout << *reinterpret_cast<float*>(dst1->ptr_to_element(Coordinates(0,1))) <<std::endl;
    std::cout << *reinterpret_cast<float*>(dst2->ptr_to_element(Coordinates(0,1))) <<std::endl;
    std::cout << *reinterpret_cast<float*>(dst3->ptr_to_element(Coordinates(0,1))) <<std::endl;
    std::cout << "src/cpu/kernels/CpuSimpleForwardKernel.cpp Runnnn" << std::endl;
