...
//
#include "kai_rhs_pack_kxn_qsi8cxp_qsi8cx_neon.h"
...
size_t kai_get_n_step_rhs_pack_kxn_qsi8cxp_qsi8cx_neon(size_t nr) {
    return nr;
}

size_t kai_get_rhs_offset_rhs_pack_kxn_qsi8cxp_qsi8cx_neon(size_t n_idx, size_t rhs_stride) {
    KAI_UNUSED(rhs_stride);
    return n_idx;
}

size_t kai_get_rhs_packed_stride_rhs_pack_kxn_qsi8cxp_qsi8cx_neon(size_t k, size_t nr, size_t kr, size_t sr) {
    KAI_UNUSED(kr);
    KAI_UNUSED(sr);
...
size_t kai_get_rhs_packed_offset_rhs_pack_kxn_qsi8cxp_qsi8cx_neon(
    size_t n_idx, size_t k, size_t nr, size_t kr, size_t sr) {
    KAI_ASSERT((n_idx % nr) == 0);

    return (n_idx / nr) * kai_get_rhs_packed_stride_rhs_pack_kxn_qsi8cxp_qsi8cx_neon(k, nr, kr, sr);
}

size_t kai_get_rhs_packed_size_rhs_pack_kxn_qsi8cxp_qsi8cx_neon(size_t n, size_t k, size_t nr, size_t kr, size_t sr) {
    const size_t num_rows = kai_roundup(n, nr) / nr;

    return num_rows * kai_get_rhs_packed_stride_rhs_pack_kxn_qsi8cxp_qsi8cx_neon(k, nr, kr, sr);
}

void kai_run_rhs_pack_kxn_qsi8cxp_qsi8cx_neon(
    size_t num_groups,   //
    size_t n,            //
    size_t k,            //
    size_t nr,           //
    ...
    for (size_t dst_row_idx = 0; dst_row_idx < dst_num_rows; ++dst_row_idx) {
        ...
        for (size_t dst_offset = 0; dst_offset < dst_num_bytes_per_row; dst_offset += kr) {
            ...
            const size_t src_offset = n0_valid_idx;
            ...
            for (size_t i = 0; i < kr; i++) {
                ...
                int8_t v = 0;
                if (k0_valid_idx < k) {
                    v = rhs[src_offset + (k0_valid_idx * rhs_stride)];
                }
