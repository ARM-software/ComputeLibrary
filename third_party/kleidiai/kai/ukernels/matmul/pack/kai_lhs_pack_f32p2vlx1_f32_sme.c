...
enum {
    MR = 2,
    KR = 1,
    MAX_M_STEP = MR * (KAI_SME_VEC_LENGTH_MAX_BYTES / sizeof(float)) / KR,
    SR = 1,
};

void kai_kernel_lhs_pack_f32p2vlx1_f32_sme(size_t height, size_t width, const void* in, void* out);

static size_t kai_get_mr_lhs_pack_f32p2vlx1_f32_sme(void) {
    return MR * kai_get_sme_vector_length_u32() / KR;
}

size_t kai_get_m_step_lhs_pack_f32p2vlx1_f32_sme(size_t mr) {
    KAI_ASSUME(mr == kai_get_mr_lhs_pack_f32p2vlx1_f32_sme());
    KAI_UNUSED(mr);
    return kai_get_mr_lhs_pack_f32p2vlx1_f32_sme();
}

size_t kai_get_lhs_offset_lhs_pack_f32p2vlx1_f32_sme(size_t m_idx, size_t lhs_stride_row) {
    KAI_ASSUME(m_idx % kai_get_mr_lhs_pack_f32p2vlx1_f32_sme() == 0);
...
size_t kai_get_lhs_packed_offset_lhs_pack_f32p2vlx1_f32_sme(size_t m_idx, size_t k, size_t mr, size_t kr, size_t sr) {
    KAI_ASSUME(m_idx % kai_get_m_step_lhs_pack_f32p2vlx1_f32_sme(mr) == 0);
    KAI_ASSUME(mr == kai_get_mr_lhs_pack_f32p2vlx1_f32_sme());
    KAI_ASSUME(kr == KR);
    KAI_ASSUME(sr == SR);
    ...
    return m_idx * kai_roundup(k, KR) * sizeof(float);
}
...
size_t kai_get_lhs_packed_size_lhs_pack_f32p2vlx1_f32_sme(size_t m, size_t k, size_t mr, size_t kr, size_t sr) {
    ...
    KAI_UNUSED(kr);
    KAI_UNUSED(sr);
    return kai_roundup(m, kai_get_mr_lhs_pack_f32p2vlx1_f32_sme()) * kai_roundup(k, KR) * sizeof(float);
}
...
void kai_run_lhs_pack_f32p2vlx1_f32_sme(
    ...
    const size_t m_step = kai_get_mr_lhs_pack_f32p2vlx1_f32_sme();
    ...
    for (size_t i_m = 0; i_m < m; i_m += m_step) {
        ...
        void* out = out_base;
        out_base += m_step * kai_roundup(k, KR) * sizeof(float);
