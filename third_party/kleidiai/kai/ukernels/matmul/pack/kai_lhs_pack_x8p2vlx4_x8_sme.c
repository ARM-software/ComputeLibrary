...
void kai_kernel_lhs_pack_x8p2vlx4_x8_sme(size_t height, size_t width, const void* in, void* out);
...
size_t kai_get_lhs_offset_lhs_pack_x8p2vlx4_x8_sme(size_t m_idx, size_t lhs_stride_row) {
...
void kai_run_lhs_pack_x8p2vlx4_x8_sme(
    ...
    for (size_t i_m = 0; i_m < m; i_m += m_step) {
        ...
        void* out = out_base;
        out_base += m_step * kai_roundup(k, KR) * sizeof(int8_t);

        for (size_t y = 0; y < height; y++) {
            in[y] = lhs_ptr + (i_m + y) * lhs_stride_row;
        }

        kai_kernel_lhs_pack_x8p2vlx4_x8_sme(
