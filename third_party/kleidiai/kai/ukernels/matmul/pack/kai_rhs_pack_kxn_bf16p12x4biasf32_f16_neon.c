...
size_t kai_get_n_step_rhs_pack_kxn_bf16p12x4biasf32_f16_neon(void) {
    return kai_nr;
}

size_t kai_get_rhs_offset_rhs_pack_kxn_bf16p12x4biasf32_f16_neon(size_t n_idx) {
    KAI_ASSUME(n_idx % kai_nr == 0);

    return n_idx * sizeof(uint16_t);
}

size_t kai_get_bias_offset_rhs_pack_kxn_bf16p12x4biasf32_f16_neon(size_t n_idx) {
    return n_idx * sizeof(uint32_t);
}

size_t kai_get_rhs_packed_offset_rhs_pack_kxn_bf16p12x4biasf32_f16_neon(size_t n_idx, size_t k) {
    KAI_ASSUME(n_idx % kai_nr == 0);

    return n_idx * (sizeof(uint32_t) + kai_roundup(k, kai_kr) * sizeof(uint16_t));
}

size_t kai_get_rhs_packed_size_rhs_pack_kxn_bf16p12x4biasf32_f16_neon(size_t n, size_t k) {
    return kai_get_rhs_packed_offset_rhs_pack_kxn_bf16p12x4biasf32_f16_neon(kai_roundup(n, kai_nr), k);
}

void kai_run_rhs_pack_kxn_bf16p12x4biasf32_f16_neon(
    ...
    uint8_t zero_bias[kai_nr * sizeof(float)];

    if (bias == NULL) {
        memset(zero_bias, 0, kai_nr * sizeof(float));
        bias_step = 0;
    }
